...
export default function TrackRepairIsland({
  ...
  const handlePay = () => {
    ...
    // Convert KES or local currency string directly to Kobo (multiply by 100)
    // The Medusa amounts are generally stored as the actual float amount (e.g. 15000), 
    // so to convert to Paystack kobo equivalents we multiply by 100.
    // Wait, ticket.total_estimate might be the Medusa raw value. In the UI it's divided by 100.
    // Actually, in the plugin backend the amount is the raw Medusa value (cents/kobo). 
    // Wait, the Storefront UI does: KES {((ticket.total_estimate || 0) / 100).toFixed(2)}
    // This means ticket.total_estimate is ALREADY in cents/kobo!
    const amountInKobo = Math.round(ticket.total_actual > 0 ? ticket.total_actual : ticket.total_estimate);
    ...
    try {
      ...
      paystack.newTransaction({
        ...
        email: ticket.customer?.email || "customer@example.com", // Add actual customer email if available or prompt
        amount: amountInKobo,
            ...
            {isLoggedIn &&
              ...
              <>
                <div className="flex-1 w-full">
                  ...
                  {paystackPublicKey && ticket.payment_status !== "captured" && ticket.payment_status !== "paid" && (
                    <div className="mt-6">
                      <button
                        onClick={handlePay}
                        disabled={isPaying}
